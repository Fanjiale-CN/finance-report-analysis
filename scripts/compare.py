...
def infer_measurement(text: str) -> dict[str, Any]:
    ...
    currency = "CNY" if any(x in text for x in ("人民币", "RMB", "Renminbi", "人民幣")) else "HKD" if any(x in text for x in ("港币", "港元", "HK$", "HKD")) else "USD" if any(x in text for x in ("US$", "USD", "dollar", "dollars")) else "unknown"
    if re.search(r"rmb['’]?000|千元|thousand yuan|人民币千元", text, re.I):
        scale = 1000.0
        unit = "thousand"
    elif re.search(r"rmb['’]?000['’]?000|百万元|millions?", text, re.I):
        scale = 1_000_000.0
        unit = "million"
        ...
        scale = 1.0
        unit = "base"
    return {"currency": currency, "scale_to_base": scale, "display_unit": unit}
...
def normalize_report(report: dict[str, Any], fx_to_cny: dict[str, float] | None = None) -> dict[str, Any]:
    measurement = infer_measurement(_text(report["source"]))
    ...
    return {"source": report["source"], "report_type": report["report_type"], "detection": report["detection"], "measurement": measurement, "normalized": normalized, "warnings": warnings}
